...
public extension SRGDataProvider {
     ...
     *  to disable them.
     */
    func medias(for vendor: SRGVendor, matchingQuery query: String?, with settings: SRGMediaSearchSettings? = nil, pageSize: UInt = SRGDataProviderDefaultPageSize, triggerId: Trigger.Id? = nil) -> AnyPublisher<MediasMatchingQuery.Output, Error> {
        let request = requestMedias(for: vendor, matchingQuery: query, with: settings)
        return paginatedObjectsTriggeredPublisher(at: Page(request: request, size: pageSize), rootKey: "searchResultMediaList", type: SRGSearchResult.self, triggerId: triggerId)
            .map { result in
     ...
     */
    func shows(for vendor: SRGVendor, matchingQuery query: String, mediaType: SRGMediaType = .none, pageSize: UInt = SRGDataProviderDefaultPageSize, triggerId: Trigger.Id? = nil) -> AnyPublisher<ShowsMatchingQuery.Output, Error> {
     ...
     */
    func shows(for vendor: SRGVendor, matchingQuery query: String, transmission: SRGTransmission = .none, pageSize: UInt = SRGDataProviderDefaultPageSize, triggerId: Trigger.Id? = nil) -> AnyPublisher<ShowsMatchingQuery.Output, Error> {
     ...
     */
    func videos(for vendor: SRGVendor, withTags tags: [String], excludedTags: [String]? = nil, fullLengthExcluded: Bool = false, pageSize: UInt = SRGDataProviderDefaultPageSize, triggerId: Trigger.Id? = nil) -> AnyPublisher<[SRGMedia], Error> {
