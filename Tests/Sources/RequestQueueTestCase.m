...
- (void)testCascadingRequests
{
    ...
    SRGRequest *videosRequest = [self.dataProvider videoTrendingWithCompletionBlock:^(NSArray<SRGMedia *> * _Nullable medias, SRGPage *page, SRGPage * _Nullable nextPage, NSError * _Nullable error) {
        [requestQueue reportError:error];
...
- (void)testGlobalCancel
{
    ...
    SRGRequest *request2 = [self.dataProvider videoSoonExpiringWithCompletionBlock:^(NSArray<SRGMedia *> * _Nullable medias, SRGPage *page, SRGPage * _Nullable nextPage, NSError * _Nullable error) {
        XCTFail(@"Completion block must not be called when the request has been cancelled");
    }];
    [requestQueue addRequest:request2 resume:YES];
