...
NSString *SRGAccessibilityLabelForYouthProtectionColor(SRGYouthProtectionColor youthProtectionColor)
{
    ...
    static NSDictionary *s_messages;
    dispatch_once(&s_onceToken, ^{
        s_messages = @{ @(SRGYouthProtectionColorYellow) : SRGDataProviderAccessibilityLocalizedString(@"Parental advisory", @"A youth protection message stated to the user if the media has the yellow protection color."),
                        @(SRGYouthProtectionColorRed) : SRGDataProviderAccessibilityLocalizedString(@"Desensitized public", @"A youth protection message stated to the user if the media has the red protection color.") };
    });
    return s_messages[@(youthProtectionColor)];
}
